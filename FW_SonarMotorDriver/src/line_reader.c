/* line_reader.c — Извлечение строк из кольцевых буферов.
 *
 * Используется UART и USB CDC. Поддерживает \r, \n, \r\n.
 * Пустые строки пропускаются. При переполнении (EOL не найден) tail
 * смещается на один байт («медленный слив»), чтобы освободить место.
 */
