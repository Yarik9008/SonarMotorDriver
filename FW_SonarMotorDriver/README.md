...
6. **Telemetry_Tick** — периодический вывод (250 Гц по умолчанию): debug=0 → cp,ec; debug=1 → полная.
...
│   └── cmd_parser.h         — парсер команд en/dis/t/kp/ki/kd/op/debug
├── src/
...
| `cmd_parser` | Парсер команд (en, dis, t=, kp=, ki=, kd=, op=, debug=)  |
...
При старте: драйвер включается при первом ответе энкодера, мотор движется в домашнюю позицию (0° + офсет) кратчайшим путём.
