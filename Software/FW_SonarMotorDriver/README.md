...
>
> **Имитатор — это та же прошивка.** Проект собирается в двух окружениях:
> `pio run` (окружение `hw`) даёт боевой образ, `pio run -e sim` — имитатор без
> энкодера и драйвера, для отладки ПО верхнего уровня без железа. Логика у них
> общая целиком, различается только слой железа — см.
> [Имитатор (env:sim)](#имитатор-envsim). Сводное описание протокола (все
> команды, телеметрия, тайминги, USART3-дублирование) — в
> [Docs/Protocol.md](../../Docs/Protocol.md); разделы «Команды (UART)» и
> «Телеметрия» ниже описывают то же самое в контексте этой прошивки.
...
- [Имитатор (env:sim)](#имитатор-envsim)
...
```
FW_SonarMotorDriver/
├── platformio.ini           — два окружения: hw (плата) и sim (имитатор)
├── include/
...
│   └── pid.h                — PID-регулятор
├── src/
│   ├── main.c               — init, главный цикл, контроллер, телеметрия (обе сборки)
│   ├── biss_c.c             — BiSS C: SPI + DMA, CRC6, разбор кадра   (только hw)
│   ├── sim_backend.c        — виртуальные энкодер и мотор             (только sim)
│   └── pid.c                — PID с anti-windup                      (обе сборки)
└── tools/
    ├── read_serial.py       — утилита чтения COM-порта
    └── test_commands.py     — автотест всех команд с логом (привязка по времени)
```

Какой файл попадёт в сборку, решает `build_src_filter` окружения: у `hw`
исключён `sim_backend.c`, у `sim` — `biss_c.c` (и не подключается библиотека
драйвера). Подробности — [Имитатор (env:sim)](#имитатор-envsim).

Собственные библиотеки проекта лежат в `lib/` и подхватываются PlatformIO
автоматически — по `#include` в исходниках, без `lib_deps`. Внешних
зависимостей у прошивки нет: сборка не ходит в реестр PlatformIO и
воспроизводится из одного каталога.

```
lib/tmc2209/                 — драйвер TMC2209
├── library.json
...
└── src/
    ...
    └── tmc2209_motor.c      — только при -DTMC2209_USE_MOTOR_FACADE

lib/sonar_proto/             — слой командного протокола
├── library.json
├── include/                 — заголовки в корне: #include "cmd_parser.h"
...
│   ├── line_reader.c
│   └── cmd_parser.c
└── tools/                   — хостовый зонд: настоящий cmd_parser.c под gcc
    ├── cmd_parser_probe.c
    └── run_cmd_parser_probe.py
```

Обе живут здесь, а не в отдельном общем каталоге, потому что потребитель у них
один — эта прошивка (имитатор `env:sim` — её же сборка). Исключение —
тестовый стенд [FW_Test_TMC2209](../FW_Test_TMC2209/README.md): он ссылается
на `lib/tmc2209` отсюда строкой `lib_deps = symlink://../FW_SonarMotorDriver/lib/tmc2209`,
чтобы проверять ровно тот драйвер, который стоит в изделии, а не копию (копии
уже расходились на сотню строк). Исходники библиотек берут `board.h` из
`include/` этой прошивки — оттуда приходят выводы, скорости и размеры буферов
(`UART_*`, `UART3_*`, `TMC2209_*`, `MOTOR_DRIVER_MODE`, `STEP_*`/`DIR_*`).
...
| `lib/tmc2209` | Драйвер (общий с тестовым стендом по ссылке): регистры, UART к TMC2209 на `USART2`, порт STM32, генерация STEP/DIR (TIM4), диагностика |
| `lib/sonar_proto` | Слой протокола: `uart.c` + `line_reader.c` + `cmd_parser.c` |
...
| `biss_c`      | BiSS C: SPI + DMA, разбор кадра, CRC6 (только сборка `hw`)                                                |
| `sim_backend` | Виртуальные энкодер и мотор вместо BiSS C и TMC2209 (только сборка `sim`)                                 |
...
Имитатор нужен для отладки ПО верхнего уровня (парсеров телеметрии, GUI,
ROS-узлов) без мотора, энкодера и силового питания: плата отвечает по UART
ровно как боевая, но вал существует только в виде модели.

Раньше это была отдельная прошивка со своей копией логики, и копии успели
разойтись — например, сектор скана через ноль (`scan=350,10,5,100`) основная
прошивка принимала, а имитатор отвечал `err:scan`. Теперь это **одна и та же
прошивка**: `main.c`, ПИД, профиль движения, скан, синхронизация, телеметрия и
парсер команд собираются из общих исходников, условий компиляции внутри логики
нет. Сборка подменяет только слой железа:

| Окружение | Слой железа | Образ |
| --------- | ----------- | ----- |
| `hw` (по умолчанию) | `src/biss_c.c` (энкодер BiSS C) + библиотека `lib/tmc2209` (драйвер) | `.pio/build/hw/` |
| `sim` | `src/sim_backend.c`: виртуальный энкодер и виртуальный шаговый двигатель | `.pio/build/sim/` |

```bash
pio run -e sim              # сборка
pio run -e sim -t upload    # прошивка той же платы
```

### Что делает модель

`src/sim_backend.c` реализует те же интерфейсы, что и настоящее железо
(`biss_c.h` и `tmc2209/tmc2209_motor.h`), поэтому `main.c` о подмене не знает:

- **вал** — положение в градусах, которое раз в тик опроса (1 кГц)
  продвигается на скомандованную скорость и серии шагов доводки; без тока в
  обмотках (`dis`, `hold=0`) вал стоит, как и на плате;
- **энкодер** — отдаёт это положение, квантуя его в отсчёты (17 бит,
  0.0027°), поэтому дискретность телеметрии та же, что у настоящего датчика;
  чтения всегда валидны;
- **драйвер** — хранит ток и микрошаг для `mcfg`, проверяет `mstep` на степень
  двойки, держит вывод ENN в HIGH (если имитатор окажется прошит в боевую
  плату, силовая часть останется выключенной).

При старте вал модели стоит на 37.5° и уходит в «дом» (0°) — видимая имитация
хоминга после сброса.

### Отличия от боевого образа

| Аспект | Боевой (`hw`) | Имитатор (`sim`) |
| ------ | ------------- | ---------------- |
| Команды, ответы, телеметрия, тайминги | — | тот же код, строки совпадают |
| ПИД, профиль `v=`/`a=`, скан, `sync=`, `hold=`, `om=` | — | тот же код, работают по модели вала |
| Баннер `boot:rst=… fw=…` | `fw=1.0.0` | `fw=1.0.0-sim` — единственная строка, по которой хост отличит имитатор |
| Позиция `cp` | энкодер BiSS C | модель вала (та же дискретность) |
| `ec` (код ошибки) | реальные ошибки энкодера | всегда 0 |
| `m` (режим) | `cl`/`ol` по состоянию энкодера | всегда `cl` |
| Диагностика (старт и `diag`) | серия чтений BiSS, возможен `err:enc` | всегда `enc:ok n=16/16 spread=0.000` |
| Фильтр выбросов (`ec:6`, поле `of`) | отбрасывает неправдоподобные чтения | выбросов нет, `of:0` |
| Защита от блокировки (`err:stall`) | выключает драйвер | не срабатывает: вал идеально следует команде |
| `irun/ihold/icur/mstep` | пишутся в чип по UART | сохраняются, отражаются в `mcfg` (`ready=1`) |
| Пин ENN | управляется драйвером | удерживается в HIGH |

> Не путать с **моделью на стороне ПК**: `Software/SonarDebugGUI/sonar_gui/simulator.py`
> — это Python-повторение прошивки для GUI и конформанс-теста
> (`protocol_test.py --sim`), оно работает без платы вообще. Образ `sim`
> прошивается в настоящий STM32 и отвечает по настоящему UART.
...
```bash
pio run                   # Сборка боевой прошивки (окружение hw)
pio run -e sim            # Сборка имитатора
pio run -e hw -e sim      # Оба образа сразу (так собирается Release/)
pio run --target upload   # Прошивка боевого образа
pio run -e sim -t upload  # Прошивка имитатора
```

Образы кладутся в `.pio/build/hw/` и `.pio/build/sim/` соответственно.
...
чтобы флаги не попали на исходники STM32Cube HAL). Библиотеки проекта из
`lib/` включают те же флаги у себя в `library.json`. Сборка должна
