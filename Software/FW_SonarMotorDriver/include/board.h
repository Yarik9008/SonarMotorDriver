...
#define ENCODER_COAST_MS        20U     /* Мс коастинга в CL без данных энкодера, затем стоп мотора */
#define BISS_DMA_TIMEOUT_MS     5U      /* Аварийный таймаут DMA-чтения BiSS (штатно ~64 мкс) */

/* -------- Диагностика энкодера перед стартом движения --------
 * При старте прошивка выполняет серию проверочных чтений BiSS-C и разрешает
 * автоматический выход в «дом» только если энкодер стабильно отвечает и
 * позиция не «плывёт». При провале мотор остаётся выключенным (err:enc),
 * позиция продолжает отслеживаться, движение — только по команде оператора. */
 ...
 * Показание, подразумевающее физически невозможный скачок позиции
 * (> ENCODER_OUTLIER_MAX_DELTA_DEG за тик — при потолке 1200°/с это 1.2°/мс),
 * отбрасывается (ec=6, в контур не попадает). Если несколько чтений подряд
 * согласно показывают новую позицию — это реальный скачок (проскальзывание,
 * восстановление связи на ходу): позиция перепривязывается. Бюджет скачка
 * масштабируется на длительность пропуска данных. */
#define ENCODER_OUTLIER_MAX_DELTA_DEG 3.0f  /* Макс. правдоподобный сдвиг за тик, град */
#define ENCODER_OUTLIER_STREAK        3U    /* Согласных чтений подряд = реальное движение */
...
/* -------- Профиль движения (настраивается на лету: v= и a=) --------
 * v — рабочий предел скорости (1..MAX_SPEED_DEG_S), a — предел ускорения.
 * a = 0 отключает рампу (скорость меняется мгновенно, поведение как раньше).
 * При a > 0 действует и тормозной конверт: подъезд к цели со скоростью,
 ...
 * Работает в closed-loop: если мотору командуется движение, а энкодер
 * показывает, что вал не следует за командой — драйвер выключается
 * (ENN = HIGH, g_enabled = 0), хост получает err:stall. Возврат в работу —
 * командой en. В open-loop (энкодер недоступен) защита не действует.
 *
 * Два независимых критерия:
 *  - Быстрый (полная блокировка): вал сдвинулся меньше STALL_STUCK_DEG при
 *    скомандованном пути ≥ STALL_FAST_CMD_DEG — вердикт уже через
 *    STALL_FAST_MS (реакция на «схватили вал рукой», а не через секунду).
 *  - Медленный (частичный стопор): вал движется, но за полное окно
 *    STALL_TIMEOUT_MS прошёл меньше STALL_MEAS_FRACTION скомандованного пути. */
#define STALL_TIMEOUT_MS        1000U  /* Полное окно медленного (частичного) стопора */
#define STALL_MIN_CMD_DEG       1.0f   /* Мин. скомандованный путь за окно для вердикта */
#define STALL_MEAS_FRACTION     0.25f  /* Факт < этой доли команды → вал заблокирован */
#define STALL_STUCK_DEG         5.0f   /* Факт. сдвиг за окно < этого → вал физически застопорен */
#define STALL_FAST_CMD_DEG      15.0f  /* Скомандованный путь для быстрого вердикта блокировки */
#define STALL_FAST_MS           200U   /* Мин. окно быстрого пути (отсев переходных процессов) */
#define STALL_IDLE_RESET_MS     100U   /* Мс без команды движения → сброс окна детекции */
...
/* SPI1 (BiSS) занимает DMA1 Ch2/Ch3 — USART3 работает через прерывания */
#define UART3_USE_DMA           0U
 ...
 * Жёсткий real-time сверху вниз:
 *   STEP (TIM4 update, счёт импульсов доводочных серий) — самый высокий;
 *   TIM_POLL (TIM2, сэмплирование энкодера 1 кГц) — под ним;
 *   DMA (SPI1/BiSS) — обслуживается между тиками TIM2;
 *   UART (USART1/USART3, их линии и DMA) — мягкая телеметрия/команды.
 * UART-линия и её RX-DMA держатся на ОДНОМ уровне: иначе DMA вытеснит
 * обработчик IDLE и uart_rx_drain() войдёт повторно, повредив приём. */
#define IRQ_PRIO_STEP           5U
#define IRQ_PRIO_TIM_POLL       6U
#define IRQ_PRIO_DMA            7U
#define IRQ_PRIO_UART           8U
