...
static uint16_t tx_count(const UartPortCtx *p)
{
    ...
    /* Обе ветви ?: приводим к uint16_t явно: без этого одна имеет тип int
       (h - t после целочисленного продвижения), а вторая — unsigned int
       (UART_TX_RING_SIZE объявлен с суффиксом U), что даёт -Wsign-compare.
       Значения заведомо меньше размера кольца, так что усечение безопасно. */
    return (h >= t) ? (uint16_t)(h - t) : (uint16_t)(UART_TX_RING_SIZE - t + h);
}
