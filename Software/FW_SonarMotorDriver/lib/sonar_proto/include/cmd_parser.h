 ...
 * - om=N     : режим выдачи телеметрии (0=по таймеру op, 1=по достижению
 *              целевой позиции, 2=оба источника).
 * - om       : запрос режима выдачи телеметрии (ответ om=N).
 ...
 * - hold=N   : удержание вала (0 = силовая часть обесточена — тихая пауза для
 *              замера, 1 = штатное удержание); цель и точка скана сохраняются.
 * - hold     : запрос удержания вала (ответ hold=0|1).
 */
...
typedef enum {
    ...
    CMD_SET_OUTPUT_MODE,    ///< Режим выдачи телеметрии (om=0|1|2)
    ...
    CMD_SET_HOLD,           ///< Удержание вала (hold=0|1)
    CMD_GET_HOLD,           ///< Запрос удержания вала (hold)
    CMD_GET_OUTPUT_MODE,    ///< Запрос режима выдачи телеметрии (om)
...
typedef struct {
    ...
    uint8_t output_mode;    /* режим выдачи телеметрии (om=): 0..2 */
    ...
    uint8_t hold;           /* удержание вала (hold=): 0=снять, 1=включить */
} Cmd_Result;
