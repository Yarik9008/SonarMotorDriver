 ...
 *
 * Реализация (tmc2209_motor.c) собирается только при -DTMC2209_USE_MOTOR_FACADE
 * в build_flags прошивки: фасад читает конфигурацию платы из её board.h, и без
 * явного запроса его код не компилируется — иначе прошивке, которая фасадом не
 * пользуется, пришлось бы объявлять чужие макросы ради сборки. Прошивка,
 * поставившая флаг, задаёт в board.h выводы (STEP/DIR/EN, линию PDN_UART) и при
 * необходимости параметры мотора; у параметров есть значения по умолчанию,
 * перечисленные в начале tmc2209_motor.c.
 */
