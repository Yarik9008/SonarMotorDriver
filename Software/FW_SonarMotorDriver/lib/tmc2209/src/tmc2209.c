                                       ...
                                       uint8_t reg, uint32_t *value)
{
    ...
    /* Вариант 7 байт: полный ответ 8 байт, но один байт (0xFF) потерян при приёме — восстанавливаем и проверяем CRC */
    if (n >= 7) {
                              ...
                              const tmc2209_io_t *io)
{
    ...
    /* PWMCONF. 0xC10D0024 — библиотечная настройка по умолчанию (применяется,
     * когда вызывающий не задал cfg->pwmconf), по полям (TMC2209 rev. 1.05,
     * §5.5.2, стр. 32): PWM_OFS=36 [7:0], PWM_GRAD=0 [15:8], pwm_freq=%01
     * [17:16], pwm_autoscale=1 [18], pwm_autograd=1 [19], freewheel=0 [21:20],
     * [23:22] reserved=0, PWM_REG=1 [27:24], PWM_LIM=12 [31:28].
     * Это не заводское содержимое регистра после сброса: по карте OTP (§5.1.1,
     * стр. 23-24) у чипа с чистой OTP (все биты = 0) PWM_REG=%1000=8 (otp2.1=0,
     * «max. 4 increments/cycle») и PWM_GRAD=14 (otp1.3..1.0=0), т. е. power-up даёт
     * 0xC80D0E24; совпадают только PWM_OFS, PWM_LIM, pwm_freq, autoscale и autograd.
     * PWM_REG — коэффициент пропорциональности регулятора амплитуды StealthChop
     * (§6.3, стр. 37): 1 = 0.5 инкремента на полуволну (самая медленная и
     * мягкая подстройка), 15 = 7.5 (самая быстрая). Даташит рекомендует
     * держать его «as small as possible» и отмечает, что при аккуратно
     * подобранных скорости и ускорении хоминга минимального PWM_REG=1 часто
     * достаточно, — поэтому значение оставлено как есть: оно лежит в допустимом
     * диапазоне 1..15 и даёт самый спокойный ход. Обратная сторона (рис. 6.4,
     * стр. 38): при большом ускорении на этапе автотюнинга AT#2 слишком
     * малый PWM_REG не успевает за изменением back-EMF; в таком случае PWMCONF
     * задаётся через cfg->pwmconf либо берётся из преднастройки
     * tmc2209_apply_stealthchop_defaults(), где стоит заводской PWM_REG=8. */
...
tmc2209_result_t tmc2209_apply_stealthchop_defaults(tmc2209_t *drv)
{
    ...
    /* Убедиться, что чоппер включён (toff > 0) */
    return shadow_modify(drv, TMC2209_REG_CHOPCONF, &drv->shadow.chopconf,
